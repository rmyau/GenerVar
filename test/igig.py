...
def task5(var):
    if var%2==1: #НЕЧЁТНЫЙ ВАРИАНТ    
        task1 =['5) Охотник, имеющий три патрона, стреляет по дичи до первого попадания или пока не израсходует все патроны.',' Составить ряд распределения числа выстрелов,',
                ...
                '. Найти М(Х), D(X), δ(X), этой случайной величины. Построить график F(X).']
        a=round(rand.uniform(0.1,0.9),1)
        b=rand.randint(3,4)
        ...
        for i in range(b):
            k2=k2+(z[i]*z[i]*y[i])
        d=k2-(k*k)
        ot1='M(x)='+str(round(k,2))+'  D(x)='+str(round(d,2))+'  δ(x)='+str(round(math.sqrt(d),2))
        k=" X= "
       ...
       """+ot1
        ...
        for i in range(4):
            k2=k2+(x[i]*x[i]*y[i])
        d=k2-(k*k)
        ot1='M(x)='+str(k)+'  D(x)='+str(k2)+'  δ(x)='+str(round(math.sqrt(d),2))        
        k=" X= "
...
def task11(var):
    if var%2==1:
        ...
        t2=['11) Диаметр D детали, изготавливаемой на станке, есть случайная величина, распределенная по нормальному закону (m = ',
            ' см, δ = ',' см). Найти интервал, в котором с вероятностью ',' будут заключены диаметры деталей.']
